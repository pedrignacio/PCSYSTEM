...
export const apiService = {
    ...
    async cancelSale(id) {
        try {
            const response = await fetch(`${API_URL}/api/ventas/${id}/cancel`, {
                method: 'POST'
            });
            if (!response.ok) {
    ...
    async initiateTerminalPayment(amount) {
        try {
            const response = await fetch(`${API_URL}/api/haulmer/payment`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.error || 'Error al iniciar pago en terminal');
            }
            
            // La respuesta debe contener el token o id de la solicitud
            return await response.json();
        } catch (error) {
            console.error('Error:', error);
            throw error;
    ...
    async checkTerminalPaymentStatus(token) {
        try {
            const response = await fetch(`${API_URL}/api/haulmer/payment/${token}`);
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.error || 'Error al consultar estado del terminal');
            }
            return await response.json();
        } catch (error) {
            console.error('Error:', error);
    ...
    async updateOrderStatus(id, estado, notas = '') {
        try {
            const response = await fetch(`${API_URL}/api/ordenes/${id}/status`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async getOrderHistory(id) {
        try {
            const response = await fetch(`${API_URL}/api/ordenes/${id}/history`);
    ...
    async createPaymentPreference(items, orderData) {
        try {
